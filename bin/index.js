...
{
	let format_str=msg.toString();

	if (format_str.length<width)
	{
		format_str+=" ".repeat(width-format_str.length);
	}
	
	switch(format)
	{	
		case "black":
			//format_str= "\x1b[30m"+format_str+"\x1b[0m";
			//reverse color
			format_str= "\x1b[37m"+format_str+"\x1b[0m";
			break
		case "red":
			format_str= "\x1b[31m"+format_str+"\x1b[0m";
			break;
		case "green":
			format_str= "\x1b[32m"+format_str+"\x1b[0m";
			break;
		case "yellow":
			format_str= "\x1b[33m"+format_str+"\x1b[0m";
			break;
		case "blue":
			format_str= "\x1b[34m"+format_str+"\x1b[0m";
			break;
		case "magenta":
			format_str= "\x1b[35m"+format_str+"\x1b[0m";
			break;
		case "cyan":
			format_str= "\x1b[36m"+format_str+"\x1b[0m";
			break;
		case "white":
			//format_str= "\x1b[37m"+format_str+"\x1b[0m";
			//reverse color
	...
	process.stdout.write(format_str);
}

function HexPad(msg,width)
{
	return msg.toString(16).toUpperCase().padStart(width,"0");
}

function debugOut(newbyte)
{
	let newchar=String.fromCharCode(newbyte);
	if (debugBufferState=="normal")
	{
		if (newchar=="\\") debugBufferState="slashed";
		else if (newchar.charCodeAt(0)>=9)
		{
			print(newchar,debugColor);
		}
	}
	else if (debugBufferState=="slashed")
	{
		...
		else if ("roygbpwl".includes(newchar))
		{
			...
			else if (newchar=="o") debugColor="orange";
			else if (newchar=="y") debugColor="yellow";
			else if (newchar=="g") debugColor="green";
			else if (newchar=="b") debugColor="blue";
			else if (newchar=="p") debugColor="purple";
			else if (newchar=="w") debugColor="white";
			else if (newchar=="l") debugColor="black";
		}						
		//Not found so treat as not slashed
		else print("\\"+newchar,debugColor);
		debugBufferState="normal";
	}
}

function key_handler(str, key)
{
	if (key.ctrl)
	{
		switch(key.name)
	...
	{
		const key_code=key.sequence.charCodeAt(0);
		//print(key_code);
	}
}
...
function LoadFiles()
{
	...
	//Directory of script not pwd since script is called from somewhere else
	//https://stackoverflow.com/questions/3133243/how-do-i-get-the-path-to-the-current-script-with-node-js
	
	//File reading
	//Old: https://stackabuse.com/reading-a-file-line-by-line-in-node-js/
	//Sync: https://stackabuse.com/read-files-with-node-js/
	const fs = require('fs');
	let lines=fs.readFileSync(__dirname+'\\..\\prog.hex', 'utf8').split("\n");
	
    for (i=0;i<lines.length;i++)
	{
		lines[i]=lines[i].slice(0,-1);	//strip CR off end
	
		if (lines[i].length!=0)
		{
			if (lines[i][0]!=':')
			{
				print("\nERROR: corrupt hex file!","red");
				process.exit();
			}
			...
			{
				print("ERROR: Unknown record type "+HexCharToInt(lines[i][7])+HexCharToInt(lines[i][8])+" in line: "+lines[i],"red");
				process.exit();
			}
		}
	}
...
{
	if ([0,10,12,13].includes(character))
...
function PrintRegs()
{
	//const col1_width=15;
	print("\nPC: ");
	print(HexPad(PC,4),"green");
	print("\n");
	print("A:  "+HexPad(A,2)+"("+RegCharFilter(A)+")\n");
	print("X:  "+HexPad(X,2)+"("+RegCharFilter(X)+")\n");
	print("Y:  "+HexPad(Y,2)+"("+RegCharFilter(Y)+")\n");
	print("SP: ");
	print(HexPad(SP,2),"cyan");
	print("\n");
	print("CA: ");
	if (CalcAddress==-1) print('(none)\n');
	else
	{
		print(HexPad(CalcAddress,4)+"\n","magenta");
	}	
}
...
const NODE_EXIT =		0xFFF1;	//Access exits node.js
		...
		switch (CalcAddress&0xFFFF)
		{
			...
			case DEBUG_HEX:
				...
				if (hexout.length==1)
				{
					...
					debugOut("0".charCodeAt(0));
					debugOut(hexout.charCodeAt(0));
				}
				else
				{
					//debugBuffer.push(hexout.charCodeAt(0));
				...
				switch (data)
				{
					...
					case TIMING_INSTR_END:
						...
						if (timingEcho)
							...
							for (i=0;i<timing_msg.length;i++)
								debugOut(timing_msg.charCodeAt(i));
						break;
	...
	if ((CalcAddress&0xFFFF)>=PERIPHERALS_BEGIN)
	{
		switch (CalcAddress&0xFFFF)
		{
			...
			case FILE_INPUT:
				...
				//	return new_byte;
				//}
				if (inputIndex>=inputBuffer.length) return 0;
				else return inputBuffer.charCodeAt(inputIndex++);
			default:
				return data;
				break;
		}
